...
try:
    ...
    df = pd.DataFrame(columns=[
        "data","campeonato","jogo","tipo",
        "linha","odd","stake",
        "ritmo","confianca","resultado"
    ])
...
with aba_entrada:
    ...
    if st.button("Salvar Entrada"):
        nova = pd.DataFrame([[data, campeonato, jogo, tipo,
                              linha, odd, stake,
                              ritmo, confianca, resultado]],
                            columns=df.columns)
...
with aba_analise:
    if len(df) < MIN_AMOSTRA:
        st.warning(f"Amostra insuficiente ({len(df)}/{MIN_AMOSTRA})")
    else:
        df["lucro"] = df.apply(calcular_lucro, axis=1)
        ...
        breakeven = 1 / odd_media
        ev = df["lucro"].mean()

        # TESTE Z
        se = sqrt(winrate * (1 - winrate) / total)
        z = (winrate - breakeven) / se
